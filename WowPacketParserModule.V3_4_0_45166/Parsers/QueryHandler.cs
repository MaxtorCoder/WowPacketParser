...
namespace WowPacketParserModule.V3_4_0_45166.Parsers
{
    public static class QueryHandler
    {
        ...
        {
            var response = packet.Holder.QueryCreatureResponse = new PacketQueryCreatureResponse();
            ...
            var titleAltLen = packet.ReadBits(11);
            var cursorNameLen = packet.ReadBits(6);
            ...
            for (var i = 0; i < 4; ++i)
            {
                if (stringLens[i][0] > 1)
                {
                    var name = packet.ReadDynamicString("Name", stringLens[i][0], i);
                    if (i == 0)
                        creature.Name = response.Name = name;
                }
                if (stringLens[i][1] > 1)
                {
                    var nameAlt = packet.ReadDynamicString("NameAlt", stringLens[i][1], i);
                    if (i == 0)
            ...
            {
                var model = new CreatureTemplateModel
                {
            ...
            var questItems = packet.ReadUInt32("QuestItems");
            var questCurrencies = 0u;
            if (ClientVersion.AddedInVersion(ClientBranch.Cata, ClientVersionBuild.V4_4_0_53627))
            ...
            {
                var questItem = new CreatureTemplateQuestItem
                {
            ...
            {
                var questCurrency = new CreatureTemplateQuestCurrency
                {
                    CreatureId = (uint)entry.Key,
            ...
            {
                var localesCreature = new CreatureTemplateLocale
                {
                    ID = (uint)entry.Key,
                    Name = creature.Name,
        ...
        public static void HandleGameObjectQueryResponse1141(Packet packet)
        {
            ...
            var questItemsCount = packet.ReadByte("QuestItemsCount");
            for (uint i = 0; i < questItemsCount; i++)
            {
                var questItem = new GameObjectTemplateQuestItem
                {
